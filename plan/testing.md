...
| User looks ("My Looks") | `tests/test_looks_phase0b02.py` | Store CRUD/duplicate/reorder/cap/dedupe; JSON round-trip + unknown-key preservation; malformed-record skip; corrupt/missing → empty; `.bak` rotation; export/import fresh-id; settings v8→v9 migration; app capture/apply round-trip, dirty tracking, **save bookmarks without auto-activating (stays on None/Live)**, **None / Live restores the live state (not the saved snapshot)**, non-fatal missing mode; panel save action |
| Auto-cycle ("shuffle") | `tests/test_autocycle_phase0b03.py` | Settings v9→v10 migration + `random_pool`/`random_interval` round-trip (junk dropped, interval clamped); `ModeTransition` snapshot-dissolve alpha; **tagged pool** (`mode:`/`look:`) toggle / All-None / valid filtering / no-repeat picking; interval clamp; reduce-motion **cut** vs **dissolve**; **saved look applied on advance**; **Next** (incl. empty-pool fill); transition lifecycle (applies live then clears); manual switch cancels an in-flight fade; stale `look:` id excluded; **`random_options` round-trip + randomize-mode-options (keeps indices valid, `preset`→Custom; off keeps defaults)**; **`random_fade` round-trip/clamp + `_adjust_fade` snap/clamp; live cross-fade for mode→mode vs frozen dissolve for looks; `0s` fade hard-cut; status-chip Mode/Look label**; fade frame + countdown overlay render; Shuffle panel click routing (incl. fade stepper) |
