...
| User looks ("My Looks") | `tests/test_looks_phase0b02.py` | Store CRUD/duplicate/reorder/cap/dedupe; JSON round-trip + unknown-key preservation; malformed-record skip; corrupt/missing → empty; `.bak` rotation; export/import fresh-id; settings v8→v9 migration; app capture/apply round-trip, dirty tracking, baseline restore, non-fatal missing mode; panel save action |
| Auto-cycle ("shuffle") | `tests/test_autocycle_phase0b03.py` | Settings v9→v10 migration + `random_pool`/`random_interval` round-trip (junk dropped, interval clamped); `ModeTransition` alpha progression; pool toggle / All-None / valid-index filtering; interval clamp; reduce-motion **cut** vs **cross-fade**; transition lifecycle leaves exactly one active mode; manual switch cancels an in-flight fade; a fade frame renders without error; Shuffle panel click routing |
