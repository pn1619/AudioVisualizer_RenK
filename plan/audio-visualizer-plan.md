...
| 30 | **macOS capture via PortAudio (`sounddevice`), phased A→B→C:** (A) spike + default input / mic; (B) BlackHole virtual device for system audio without a custom driver; (C) optional ScreenCaptureKit tap later. Mac-only dep in `requirements-mac.txt`; implementation behind `AudioSource` in `capture_mac.py` + factory — never in `requirements.txt` or Windows `capture.py`. Spike: `tools/mac/spike-capture.py` | macOS has no WASAPI loopback; PortAudio is the proven cross-platform seam; BlackHole matches user expectation before investing in SCK; spike-first de-risks the #1 port failure (decision mirrors Windows Phase 0.5) |
