...
| **Fractal Tree** | `fractal_tree` | 95 | Renders the concept artwork **itself** (bundled PNG, fit + cached, black bg keyed transparent) as the static tree; lights only the tree — shape-matched flower glow, frequency body glow, and energy running up the branches (earlier procedural `test_tree`/`test_tree2` dropped) | bass / treble / onset / rms | Phase 0B-d |
...
| 27 | **Pull-request-only `main`, enforced by a GitHub ruleset** (`Protect main`): require a PR before merging (0 approvals — the solo owner can't self-approve, so the *merge* is the gate), require the `build` CI check, block force-pushes/deletion; **owner `pn1619` (Repository admin) on the bypass list** keeps force-merge. **AI agents never push/merge to `main`**: they branch `feature/<topic>`, and create commits/pushes/PRs **only when the owner explicitly asks**, never merging/approving. Full policy in `plan/git-and-versioning.md` §1.1 (mirrored in the rule + skill) | The owner wants tighter control over development (esp. AI-agent changes): every change is reviewed via PR and merged by a human, CI stays a hard gate, yet the owner retains an emergency override. A solo repo can't require self-approval, so the merge action + a bypass-limited admin is the workable equivalent |
| 28 | **macOS port in parallel, same repo** — isolated `requirements-mac*.txt` + `tools/mac/*.sh`; shared `src/`/`tests/`; Windows `requirements.txt` + `tools/*.ps1` + CI unchanged. macOS capture TBD behind `AudioSource`; no mac-only pip deps in Windows requirements. Policy: `plan/macos-port.md` | Owner develops the macOS port on Mac without breaking the shipped Windows app; strict separation of deps/tooling prevents `pyaudiowpatch`/WASAPI assumptions from leaking into mac setup or vice versa |
| 29 | **Platform-encoded `PP` in `PP.FF.BB`:** Windows **`00`–`09`**; macOS **`A0`–`F0`** (one hex letter A–F per major mac milestone; `FF`/`BB` unchanged). Tags `v<version>` per platform (`v00.0B.2D` vs `vA0.00.00`); `APP_VERSION` stays Windows-canonical on `main` until `APP_VERSION_MAC` (or runtime pick) for mac builds. Full rules: `plan/git-and-versioning.md` §3.1 | Parallel releases without tag/version collisions; mac line is obvious in git and changelogs while Windows numbering stays untouched |
| 30 | **macOS capture via PortAudio (`sounddevice`), phased A→B→C — all shipped:** (A) mic / default input `capture_mac.py`; (B) BlackHole/aggregate for system audio; (C) driver-free **ScreenCaptureKit** tap `capture_mac_native.py` (opt-in "System Audio (native tap)", needs Screen Recording permission). Selected by `source_factory.py` (`sys.platform` + `MAC_SYSTEM_AUDIO_SOURCE_ID`); all fail-soft behind `AudioSource`. Mac-only deps (`sounddevice`, pyobjc) in `requirements-mac.txt` only. Spike: `tools/mac/spike-capture.py` | macOS has no WASAPI loopback; PortAudio is the proven cross-platform seam; BlackHole matches user expectation; SCK adds a zero-install option. Windows `capture.py`/`requirements.txt` never see mac deps |
| 31 | **macOS packaging via PyInstaller `.app`** (`AudioVisualizer-mac.spec` + `tools/mac/build-app.sh`), parallel to the Windows `.exe` spec/`build-exe.ps1`. Bundles PortAudio + pyobjc frameworks + assets, generates `.icns`, writes `Info.plist` (mic usage, `LSMinimumSystemVersion 13.0`), self-tests the app. Unsigned for now. `APP_VERSION_MAC` drives `CFBundleVersion` | Mirrors the Windows packaging seam so both platforms ship from the same repo without cross-contamination; signing/notarization deferred (recorded as open question) |
