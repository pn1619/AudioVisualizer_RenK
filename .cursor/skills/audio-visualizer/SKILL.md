...
- Top control bar (auto-hides in fullscreen): a **`Menu`** dropdown groups **Start/Stop, Fullscreen, Quit** (v00.09.01); plus ‹ / › mode + mode picker, sensitivity/smoothing/size/speed −/+ with chips, color/option dropdowns, `Motion`, and the `RenK` / `About` buttons. Positions come from `ui/layout.py` (size-relative; resize-safe).
